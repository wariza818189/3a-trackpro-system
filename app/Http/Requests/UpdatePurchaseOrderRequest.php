...
class UpdatePurchaseOrderRequest extends FormRequest
{
    ...
    {
        return [
            'expected_revision' => ['bail', 'required', 'string', 'regex:'.self::REVISION_PATTERN],
    ...
    {
        $normalized = [];
        $revision = $this->input('expected_revision');
        if (is_string($revision)) {
            $normalized['expected_revision'] = strtolower(trim($revision));
        }
    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            ...
            sort($topLevel);
            $allowed = ['_method', '_token', 'expected_revision', 'items', 'notes', 'supplier_name'];
