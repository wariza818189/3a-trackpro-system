...
Current development focus: **Purchase Order (former #25 / #25A foundation)**

Status: **In Progress**

The Purchase Order schema/model foundation is being implemented. #25A is not
complete, and the broader Purchase Order workflow is not yet operational.
...
| PROCUREMENT | Purchase Order | Create and manage purchase orders with supplier snapshot, ordered quantities, expected unit costs, and pending status. | TBD | — | — | In Progress | Working purchase-order creation and management | #25A schema/model foundation is currently in progress. MySQL schema closeout is not complete, and no route/controller/service/UI exists yet. |
| PROCUREMENT | Low-Stock Prioritization | Prioritize initialized active low/out-of-stock variants for purchase-order planning and distinguish uncovered from already covered demand. | TBD | — | — | Not Started | Working prioritized PO-planning list | Existing low-stock views do not calculate procurement coverage. Former #25B has not begun. |
