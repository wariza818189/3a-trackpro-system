...
| PROJECT PLANNING | Client Requirements | Document an assumed hardware-store sales and inventory operating scenario | Jonel Layupan | 09/09/2026 | 09/10/2026 | Completed | Requirements document | The academic project documents an assumed hardware-store operating scenario for sales, inventory, procurement, damaged receiving, and reporting; it is not newly verified client evidence. |
...
| SALES HISTORY | Void | Void an existing sale with authorization | ROBERT JAMES WARIZA | 10/07/2026 | 10/08/2026 | Completed | Working controlled sale void feature | Active Admin only; full completed-Sale void requires a normalized reason (maximum 1,000 characters), preserves the original transaction/payment evidence, restores exact stock with one SALE_VOID movement per SaleItem, writes one privacy-safe SALE_VOIDED AuditLog, and rejects repeat voids. No partial void, unvoid, refund, or cash-out workflow. Phase C guarded MySQL concurrency passed 2 tests / 93 assertions. |
...
| AUDIT TRAIL | Record Activity | Record critical user and system activities | ROBERT JAMES WARIZA | 09/26/2026 | 10/11/2026 | Completed | Working audit logging system | Production transactional event scope: USER_CREATED, USER_UPDATED, USER_ROLE_CHANGED, USER_DISABLED, USER_REACTIVATED, USER_PASSWORD_RESET, and SALE_VOIDED. The Admin-only viewer/filter UI is implemented. Stock Correction remains movement-only under FR-CORR-04; CORRECTION StockMovement is its evidence, with no duplicate AuditLog. Login/logout logging is not required. |
